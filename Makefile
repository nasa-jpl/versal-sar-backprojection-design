...
package: ${PL_BUILD_DIR}/dma_stride_controller.xo ${PL_BUILD_DIR}/dma_pkt_router.xo ${AIE_BUILD_DIR}/libadf.a ${HOST_BUILD_DIR}/${HOST_EXE} ${XSA_BUILD_DIR}/${XSA} 
...
pl: ${PL_BUILD_DIR}/dma_stride_controller.xo ${PL_BUILD_DIR}/dma_pkt_router.xo
...
# AIE simulation target. If the CSV file for input from PL doesn't exist, run
# plsim_stride to generate it.
aiesim: ${AIE_BUILD_DIR}/libadf.a
	@AIE_SWITCHES=$$(grep '^#define AIE_SWITCHES' ${PROJECT_DIR}/design/common.h | awk '{print $$3}'); \
	MAX_SWITCH_IDX=$$(($${AIE_SWITCHES} - 1)); \
	if [ ! -f ${PLSIM_BUILD_DIR}/plsimulator_output/plio_to_aie_switch_0_$${MAX_SWITCH_IDX}.csv ]; then \
	    $(MAKE) plsim_stride; \
	fi
...
# console in AIE kernel code and also generates profile information. If the CSV
# file for input from PL doesn't exist, run plsim_stride to generate it.
...
${PL_BUILD_DIR}/dma_stride_controller.xo: design/pl/dma_stride_controller.cpp design/pl/dma_stride_controller.h design/pl/stride_controller_config.cfg ${PROJECT_DIR}/design/common.h
	...
	echo -e "\n### DMA STRIDE CONTROLLER ###" >> ${PROJECT_DIR}/design/system_cfgs/system.cfg; \
	echo "nk=dma_stride_controller:1:dma_stride_controller_0" >> ${PROJECT_DIR}/design/system_cfgs/system.cfg; \
	echo -e "\n# Connect PL kernel's pl_stream_out to AIE graph's plio_stride_in_0_0" >> ${PROJECT_DIR}/design/system_cfgs/system.cfg; \
	echo "stream_connect=dma_stride_controller_0.pl_stream_out:ai_engine_0.plio_stride_in_0_0" >> ${PROJECT_DIR}/design/system_cfgs/system.cfg; \
	echo -e "\n# System port connection linking dma_stride_controller_0 instance to mem resource" >> ${PROJECT_DIR}/design/system_cfgs/system.cfg; \
	echo "sp=dma_stride_controller_0.ddr_mem:DDR" >> ${PROJECT_DIR}/design/system_cfgs/system.cfg; \
...
# Building the XSA
${XSA_BUILD_DIR}/${XSA}: ${PL_BUILD_DIR}/dma_stride_controller.xo ${PL_BUILD_DIR}/dma_pkt_router.xo ${AIE_BUILD_DIR}/libadf.a design/system_cfgs/system.cfg
	...
	v++ -g -l --platform ${PLATFORM} -t ${TARGET} \
		...
		${PROJECT_DIR}/${PL_BUILD_DIR}/dma_stride_controller.xo \
