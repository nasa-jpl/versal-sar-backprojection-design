...
The STS kernel receives its data from the ARM processor through 5 GMIO ports
...
All Image Reconstruction kernels receive its data from the ARM processor
...
After the Image Reconstruction kernels has been executed \textit{PULSES} times,
the ARM processor will read out all Image Reconstruction kernel's
\textit{img\_out} buffer in a specific way to stitch together the total focused
image.
...
An AIE design must include a dataflow graph that guides the AIE compiler in
interconnecting kernels internally and linking them to external domains within
the Versal chip. For more information on AIE graphs, see Section
\ref{subsec:aie_graphs}.

The general flow of data originates from the ARM processor. The ARM processor
provides the antenna X, Y, and Z coordinates, as well as the range to scene
center from the antenna and the range compressed samples to the STS kernel. Due
to the limited number of GMIO input ports (max of 32), the graph is configured
to forward this data to the STS kernel and that graph broadcasts the STS kernel
output to all Image Reconstruction kernels.

Data originates from the ARM processor, which provides the STS kernel with
antenna X, Y, and Z coordinates, range to scene center, and range-compressed
samples. Due to the GMIO's limited input ports (maximum 32), the AIE graph was
designed to utilize broadcasting to efficiently stream this data from the STS
kernel to all Image Reconstruction kernels. However, a separate challenge
arises with the target scene pixel coordinates for the scene of interest. Each
Image Reconstruction kernel needs its own unique set of these pixel coordinates
so it can focus its own unique portion of the overall image. Currently, this
data is directly transmitted from the ARM processor to each Image
Reconstruction kernel since the data distribution from the ARM processor is
easier round-robin to the appropriate kernels. However, this direct
communication approach poses a significant limitation: each Image
Reconstruction kernel consumes two additional GMIO ports (one GMIO for X/Y and
one for Z). Consequently, only eight Image Reconstruction kernels can be used
alongside a single STS kernel within the 32-port limit (totaling 21 GMIO
inputs). While 11 GMIO ports remain unused, the system's reliance on SIMD
processing (handling 16 elements simultaneously) restricts the number of Image
Reconstruction kernels to powers of two. The next power of two (16  Image
Reconstruction kernels) would equal a total of 37 GMIO input ports needed for
the graph design, exceed the available input GMIO ports. To address this
limitation and allow for a greater number of Image Reconstruction kernels, a
more sophisticated solution is needed than simple direct communication. Future
work aims to eliminate all GMIO ports going into and out of Image
Reconstruction kernels so they can be instantiated 100's of times over to
handle much higher workloads in parallel. See Section \ref{sec:future_work} for
possible ways of achieving this. 

The Vitis Analyzer tool was used to generate a block diagram of the designed
AIE graph (shown in figure \ref{fig:aie_graph}).
