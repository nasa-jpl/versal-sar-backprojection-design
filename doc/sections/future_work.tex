...
utilized, since only one BP Cluster can be driven at a time. Future work should
address this constraint by exploring one of the following paths: 
...
\begin{itemize}
    \item \textbf{Kernel Prototyping:} Create multiple kernel prototypes that
        ...
        each target number of BP Clusters (\texttt{AIE\_SWITCHES}).
...
allowing simultaneous data transfers to multiple BP Clusters and significantly
improving throughput. Work on this front was deferred due to funding and time
constraints but remains a key area for enhancing system scalability and
performance when using the \textit{pl\_stride} branch.
