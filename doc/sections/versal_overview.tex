...
An AIE program requires a Data Flow Graph (DFG) specification in C++, known as
an Adaptive Data Flow (ADF) graph. This graph defines a network of one or more
AIE kernels connected by data streams. Figure \ref{fig:aie_graph} shows an
example of such a graph.

The ADF graph interfaces with the programmable logic (PL), global memory, and
the host processor through dedicated constructs. For example, the
\textit{input\_plio} and \textit{output\_plio} port objects enable streaming
connections to and from the PL, while the \textit{input\_gmio} and
\textit{output\_gmio} port objects provide memory-mapped connections to global
memory (e.g. DDR). Additionally, Runtime Parameter (RTP) objects allow you to
set up and control kernel parameters during graph execution.

A graph can include multiple kernels and various input/output ports. Kernels
are connected to each other and to these ports via defined connections.
Execution begins once the required data samples—matching the buffer or stream
sizes expected by the kernels—are available, and the graph produces output
samples accordingly \cite{AMD_Versal_UG1079}.

Internally, the interconnections between AIE tiles use an AXI4 Stream fabric.
However, from the developer’s perspective, these connections can be treated
either as continuous streams or as buffer-based windows. In buffer mode
(window-based I/O), the AIE toolflow abstracts the underlying AXI4 Stream
mechanism by requiring that an entire buffer be filled before the kernel can
process or release the data to the next stage.
