...
The MATLAB implementation is used primarily to generate and manipulate the
phase history data serving as input to the Versal platform. Using Gorham’s SAR
dataset and mathematical formulation~\cite{Gorham}, the MATLAB code produces
post-processed iFFT'd time-domain data after zero-padding in the frequency
domain to achieve the desired range resolution. This preprocessing ensures that
the data provided to the Versal hardware accurately represents the desired
input conditions while bypassing the need for on-chip FFT and iFFT operations.
The MATLAB outputs are also used as a reference for verifying the accuracy of
the AI Engine backprojection results (shown in Figure~\ref{fig:matlab_img}).
The MATLAB code is shown below in listing~\ref{lst:matlab_code}.

% -----------------------------------------------------------------------------
% Generate MATLAB image from GOTACH dataset (60 degrees)
% -----------------------------------------------------------------------------
\begin{figure}[!hbtp]
  \begin{center}
    \includegraphics[width=\textwidth]{figures/MATLAB_GOTCHA_60Deg.jpg}
  \end{center}
  \caption{Generated MATLAB image from above code (60 degrees).}
  \label{fig:matlab_img}
\end{figure}
% -----------------------------------------------------------------------------
...
caxis(caxis + 20); % Shift color scale
