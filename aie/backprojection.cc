...
//    bool tlast;
//    uint32 cnt;
//    for(unsigned i=0; i<IMG_SOLVERS; i++) {
//        header = readincr(in);
//        id = header & 0x1F;
...
//        while(!tlast) {
//            if (id == 0) {
//                writeincr(out, readincr(in, tlast), new_tlast);
//            }
//            cnt++;
//        }
//    }
            ...
            for(int i=0; i<16; i++) chess_prepare_for_pipelining {
                ...
                if (low_idx_int_vec.get(i) >= low_px_idx_bound && high_idx_int_vec.get(i) <= high_px_idx_bound) {
                    ...
                    if(prev_in_loop) {
                        ...
                        // and this is the first time in the loop, send all 
                        // contents from prior kernel to next kernel.
                        if (m_id != 0) {
                            bool tlast = false;
                            while(!tlast) {
...
//ImgReconstructA::ImgReconstructA(int id)
//: m_id(id)
//, m_iter(0)
//, m_prev_low_rc((cfloat){0.0f, 0.0f})
//, m_prev_img_val((cfloat){0.0f, 0.0f})
//{}
//
//ImgReconstructB::ImgReconstructB(int id)
//: m_id(id)
//, m_iter(0)
//, m_prev_low_rc((cfloat){0.0f, 0.0f})
//, m_prev_img_val((cfloat){0.0f, 0.0f})
//{}
//
//ImgReconstructC::ImgReconstructC(int id)
//: m_id(id)
//, m_iter(0)
//, m_prev_low_rc((cfloat){0.0f, 0.0f})
//, m_prev_img_val((cfloat){0.0f, 0.0f})
...
//// ONLY ONE THAT IS DIFF FROM B AND C
//void ImgReconstructA::img_reconstruct_kern_a(input_buffer<float, extents<ST_ELEMENTS>>& __restrict slowtime_in,
//                                             input_async_buffer<TT_DATA, extents<(TP_POINT_SIZE/4)+1>>& __restrict rc_in,
//                                             output_stream<TT_DATA>* __restrict img_out,
//                                             int32 &rtp_img_elem_cnt_out) {
//
...
//                    }
//                    //} else {
//                    //    // If this is the first time in the loop, send all contents from prior
//                    //    // kernel to next kernel
//                    //    bool tlast = false;
//                    //    while(!tlast) {
//                    //        writeincr(img_out, readincr(img_in, tlast));
//                    //    }
...
//void ImgReconstructB::img_reconstruct_kern_b(input_buffer<float, extents<ST_ELEMENTS>>& __restrict slowtime_in,
//                                             input_async_buffer<TT_DATA, extents<(TP_POINT_SIZE/4)+1>>& __restrict rc_in,
//                                             input_stream<TT_DATA>* __restrict img_in, 
//                                             output_stream<TT_DATA>* __restrict img_out,
//                                             int32 &rtp_img_elem_cnt_out) {
...
//
//// EXACT SAME AS B
//void ImgReconstructC::img_reconstruct_kern_c(input_buffer<float, extents<ST_ELEMENTS>>& __restrict slowtime_in,
//                                             input_async_buffer<TT_DATA, extents<(TP_POINT_SIZE/4)+1>>& __restrict rc_in,
//                                             input_stream<TT_DATA>* __restrict img_in, 
//                                             output_stream<TT_DATA>* __restrict img_out,
//                                             int32 &rtp_img_elem_cnt_out) {
